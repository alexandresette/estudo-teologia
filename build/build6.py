...
# Portão de entrada (fora da biblioteca): vídeo em loop gerado com Veo
# (build/gate/gemini_gen.py) -- um monte com uma porta de pedra no topo,
# referência ao monte Sinai. O clipe bruto do Veo (8s) foi cortado do
# letterbox e transformado num loop "ping-pong" (ida + volta invertida,
# ~16s) via ffmpeg pra garantir emenda perfeita sem depender de o
# primeiro e o último frame do clipe baterem sozinhos.
_GATE_DIR = "/home/claude/estudo-teologia/build/gate"
gate_video_b64 = base64.b64encode(open(_GATE_DIR + "/gate-loop.mp4", "rb").read()).decode("ascii")
gate_poster_b64 = base64.b64encode(open(_GATE_DIR + "/gate-poster.jpg", "rb").read()).decode("ascii")
...
.bc-body{position:absolute;left:8%;right:8%;top:50%;transform:translateY(-50%);text-align:center;z-index:1}
.bc-orn{display:block;color:var(--gold-soft);font-size:clamp(14px,1.5cqw,24px);margin-bottom:.6em}
.bc-title{display:block;color:var(--gold-soft);font-family:'Cinzel',"Iowan Old Style","Palatino Linotype",Palatino,Georgia,serif;font-weight:700;font-size:clamp(9.5px,1.05cqw,16px);line-height:1.34;letter-spacing:0;text-shadow:0 1px 0 rgba(0,0,0,.45),0 -1px 0 rgba(255,255,255,.1)}
.bc-rule{width:32%;height:2px;background:var(--gold-soft);margin:.8em auto 0;opacity:.8;display:block}
.bc-frame{position:absolute;inset:7%;border:1px solid rgba(217,185,104,.55);pointer-events:none;z-index:0}
.bc-frame::before{content:'';position:absolute;inset:3px;border:1px solid rgba(217,185,104,.22)}
.bc-corner{position:absolute;font-size:9px;line-height:1;color:rgba(217,185,104,.8);pointer-events:none;z-index:0}
.bc-corner.tl{top:calc(7% - 4.5px);left:calc(7% - 4.5px)}
.bc-corner.tr{top:calc(7% - 4.5px);right:calc(7% - 4.5px)}
.bc-corner.bl{bottom:calc(7% - 4.5px);left:calc(7% - 4.5px)}
.bc-corner.br{bottom:calc(7% - 4.5px);right:calc(7% - 4.5px)}
...
<body>
<script>(function(){try{if(sessionStorage.getItem('estudoGateUnlocked')==='1'){document.write('<style>#gate{display:none!important}</style>');}}catch(e){}})();</script>
<div id="gate">
  <video id="gateVideo" autoplay muted loop playsinline preload="auto" poster="data:image/jpeg;base64,__GATE_POSTER__">
    <source src="data:video/mp4;base64,__GATE_VIDEO__" type="video/mp4">
  </video>
  ...
  function bookCoverHTML(b){
    return '<div class="bcover '+b.theme+'" data-id="'+b.id+'" tabindex="0" role="button" '+
      ...
      'onkeydown="if(event.key===\\'Enter\\'||event.key===\\' \\'){event.preventDefault();openBook(\\''+b.id+'\\')}">'+
        '<span class="bc-frame"></span>'+
        '<span class="bc-corner tl">&#9670;</span><span class="bc-corner tr">&#9670;</span>'+
        '<span class="bc-corner bl">&#9670;</span><span class="bc-corner br">&#9670;</span>'+
        '<span class="bc-body">'+
...
out = (out.replace("__LANDING_ART__", landing_b64)
          ...
          .replace("__GATE_VIDEO__", gate_video_b64)
          .replace("__GATE_POSTER__", gate_poster_b64))
